...
class Queries extends Conn{
    ...
    if(count($attrArray) != count($valueArray)){ return 'Incorrect input'; }

    $stringinsert = 'INSERT INTO '.$table.'('.$attr.') VALUES ('.$value.')';

    return $stringinsert;
    $pdoStatement=$this->connect()->prepare($stringinsert);
    // $pdoStatement->bindParam(':matric', $matric);
	 $status = $pdoStatement->execute();
...
print_r($tt->insert(['`name`','`email`','`password`','`role`'],['shola','abc@gmail.com','jjj','user'],'users')); 
